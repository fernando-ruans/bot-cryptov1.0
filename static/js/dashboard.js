...
class SimpleTradingDashboard {    constructor() {
    ...
    formatPrice(price, symbol) {
        ...
        this.updateCurrentPrice();        // Auto-refresh a cada 30 segundos
        setInterval(() => {
            this.loadPortfolio();
            this.loadActiveTradesStatus();
        ...
        // Atualizar preços a cada 5 segundos (mais frequente)
        setInterval(() => {
            this.updateCurrentPrice();
        }, 5000);
        
        // Atualizar trades ativos a cada 10 segundos
        setInterval(() => {
            this.loadActiveTradesStatus();
        }, 10000);
    ...
    async generateSignal() {
        ...
        try {
            ...
            const data = await response.json();
              if (data.success && data.signal) {this.currentSignal = data.signal;
    ...
    displayCurrentSignal(signal) {const card = document.getElementById('currentSignalCard');
    ...
    startMultiSymbolPriceMonitoring(activeTrades) {
        ...
        try {
            ...
            // Silencioso - erro de rede não é crítico para funcionamento
        }
    }
    ...
    stopMultiSymbolPriceMonitoring() {
        ...
        container.innerHTML = activeTrades.map(trade => {            const currentPrice = parseFloat(trade.current_price || trade.entry_price);
            const entryPrice = parseFloat(trade.entry_price);
            const pnlPercent = ((currentPrice - entryPrice) / entryPrice * 100).toFixed(2);
            const pnlValue = this.formatPrice(Math.abs(currentPrice - entryPrice), trade.symbol);
    ...
    }    async closeTrade(tradeId) {if (!confirm('Tem certeza que deseja fechar este trade?')) {
            return;
        }
    ...
    displayTradesHistory(trades) {
        ...
        // Atualizar preços em tempo real// Sempre armazenar o preço atualizado
    ...
    handleTradeUpdate(data) {
        // Atualizar trades em tempo realthis.loadPortfolio();
        this.loadActiveTradesStatus();
        
        // Mostrar notificação de atualização
        if (data.type === 'stop_loss_hit') {
            this.showNotification(`🛑 Stop Loss atingido para ${data.symbol}`, 'warning');
    ...
    handleNewSignal(data) {// Só mostrar notificação se for para o ativo atual
        if (data.symbol === this.currentSymbol) {
    ...
    }handleTradeOpened(data) {this.showNotification(
            `📊 Trade ${data.trade_type} aberto para ${data.symbol} por $${data.entry_price}`,
            'success'
        );
        
        this.loadPortfolio();
        this.loadActiveTradesStatus(); // Isso reiniciará o monitoramento multi-símbolos
    }    handleTradeClosed(data) {const pnlText = data.pnl >= 0 ? `+$${this.formatPrice(data.pnl, this.currentSymbol)}` : `-$${this.formatPrice(Math.abs(data.pnl), this.currentSymbol)}`;
    ...
    handlePortfolioUpdate(data) {// Atualizar dados do portfolio sem fazer nova requisição
        this.portfolio = data.portfolio;
        this.updateStatsDisplay();
        ...
        if (data.significant_change) {
            ...
            if (Math.abs(pnlChange) > 10) { // Apenas para mudanças > $10
        ...
        tradeCards.forEach(card => {
            const symbolElement = card.querySelector('.trade-symbol');
    ...
    updateActiveTradesPricesVisual(newPrice) {
        // Atualizar preços em cards de trades ativos com animações visuais
        const activeTradesContainer = document.getElementById('activeTradesList');
        if (!activeTradesContainer) return;
        ...
        tradeCards.forEach(card => {
            // Encontrar elementos de preço atual no card
            ...
            if (symbolElement && symbolElement.textContent.trim() === this.currentSymbol) {
                // Animar indicações de P&L baseadas na mudança de preço
                const lastPrice = this.lastPrices[this.currentSymbol] || newPrice;
                const isUpward = newPrice > lastPrice;
                
                // Adicionar efeito visual baseado na direção do preço
                if (isUpward) {
                ...
                // Animar elementos de preço específicos
    ...
    }    updateMarketDisplay(marketData) {const lastPrice = this.lastPrices[marketData.symbol] || marketData.price;
    ...
    displayCurrentPrice(price) {
        ...
        // Atualizar outros elementos de preço na interface com animações
        ...
        // Chamar método de melhoramento de exibição de preços
    ...
    addTradeCardAnimations() {
        // Adicionar animações aos cards de trades
    ...
    expandDataAfterStartup() {
        setTimeout(() => {
            fetch('/api/expand_data', { method: 'POST' })
                ...
                .then(data => {
                    if (data.success) {this.showNotification('Expandindo dados de mercado em background...', 'info');
                    }
                })
                .catch(error => {
