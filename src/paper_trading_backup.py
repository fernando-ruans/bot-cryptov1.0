...
class PaperTrade:
    ...
    def update_price(self, current_price: float) -> bool:
        ...
        # Verificar stop loss e take profit com logs detalhados
        if self.trade_type == 'buy':
            # Trade de compra
            if current_price <= self.stop_loss:
                logger.info(f"🛑 Stop Loss ativado para {self.symbol} (BUY): ${current_price:.2f} <= ${self.stop_loss:.2f}")
                self._close_trade(self.stop_loss, 'stop_loss')
                return True
            elif current_price >= self.take_profit:
                logger.info(f"🎯 Take Profit ativado para {self.symbol} (BUY): ${current_price:.2f} >= ${self.take_profit:.2f}")
                self._close_trade(self.take_profit, 'take_profit')
                return True
        else:
            # Trade de venda
            if current_price >= self.stop_loss:
                logger.info(f"🛑 Stop Loss ativado para {self.symbol} (SELL): ${current_price:.2f} >= ${self.stop_loss:.2f}")
                self._close_trade(self.stop_loss, 'stop_loss')
                return True
            elif current_price <= self.take_profit:
                logger.info(f"🎯 Take Profit ativado para {self.symbol} (SELL): ${current_price:.2f} <= ${self.take_profit:.2f}")
                self._close_trade(self.take_profit, 'take_profit')
                return True
    ...
    def to_dict(self) -> Dict:
        ...
        if self.exit_timestamp:
            ...
            duration = str(datetime.now() - self.timestamp)
        ...
        return {
            ...
            'max_favorable_excursion': self.max_price - self.entry_price if self.trade_type == 'buy' else self.entry_price - self.min_price,
            'max_adverse_excursion': self.entry_price - self.min_price if self.trade_type == 'buy' else self.max_price - self.entry_price
        }

class PaperTradingManager:
    """Gerenciador de trades virtuais (paper trading)"""
    
    def __init__(self, market_data_manager, realtime_updates=None):
        self.market_data = market_data_manager
        self.realtime_updates = realtime_updates  # Para notificações em tempo real
    ...
    def update_prices(self):
        ...
        for trade_id in closed_trades:
            if trade_id in self.active_trades:
                del self.active_trades[trade_id]    def _process_closed_trade(self, trade: PaperTrade):
        ...
        result_text = "LUCRO" if trade.pnl > 0 else "PREJUÍZO"
        
        logger.info(f"🔚 Paper trade fechado: {trade.symbol} {trade.trade_type.upper()}")
        logger.info(f"   {result_emoji} Resultado: {result_text}")
        logger.info(f"   📊 P&L: ${trade.pnl:.2f} ({trade.pnl_percent:.2f}%)")
        logger.info(f"   💰 Entry: ${trade.entry_price:.2f} → Exit: ${trade.exit_price:.2f}")
        logger.info(f"   🎯 Motivo do fechamento: {trade.exit_reason}")
        logger.info(f"   ⏱️ Duração: {trade.exit_timestamp - trade.timestamp}")
        logger.info(f"   📈 Preço máximo: ${trade.max_price:.2f}")
        logger.info(f"   📉 Preço mínimo: ${trade.min_price:.2f}")
        logger.info(f"   🎲 Confiança original: {trade.signal_confidence:.1%}")
        
        # Log do progresso geral do portfólio
        win_rate = self.winning_trades / max(len(self.trade_history), 1) * 100
        logger.info(f"📊 Portfolio Stats: {self.winning_trades}W/{self.losing_trades}L (WR: {win_rate:.1f}%) | P&L Total: ${self.total_pnl:.2f}")
