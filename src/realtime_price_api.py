...
class RealTimePriceAPI:
    ...
    def _start_rest_feed(self):
        """Feed REST como backup e para forex"""
        while self.running:
            try:
                # Atualizar preços crypto via REST (backup)
                self._update_crypto_prices_rest()
                
                # Atualizar preços forex via REST rápido
                self._update_forex_prices_rest()
    ...
    def _update_forex_prices_rest(self):
        """Atualizar preços forex via API rápida"""
        try:
            # Usar API alternativa mais rápida para forex
            forex_pairs = ['EURUSD', 'GBPUSD', 'USDJPY', 'AUDUSD', 'USDCAD']
            
            for pair in forex_pairs:
                # API de exemplo - você pode usar outra mais rápida
            ...
            logger.debug(f"Erro REST forex: {e}")
    ...
    def _fetch_price_immediate(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            # Para forex - usar API rápida alternativa
            else:
                # Implementar API de forex rápida aqui
                pass
