...
def main():
    ...
    def test_signal_gen():
        ...
        if can_generate:
            ...
            signal = signal_gen.generate_signal('BTCUSDT')
            print(f"   Sinal forçado: {signal}")
        
        return signal
    
    signal = test_step("Signal Generator", test_signal_gen)
    
    print(f"\n🎯 RESULTADO FINAL: {signal}")
    ...
    if signal is None:
        ...
        print(f"   DataFrame shape: {df_with_indicators.shape}")        print(f"   Latest close price: ${df_with_indicators['close'].iloc[-1]:.2f}")
